 ...
 */
public class PolynomialFeatures extends FeatureSet {
	...
	private double[] values = new double[OBJECTS*DEPTH + 16 + 120]; // Feature values
	...
	public FeatureSet extract(Game game, MOVE move) {
		PolynomialFeatures features = new PolynomialFeatures();
	...
	private void setValues(Game game, MOVE move) {
		...
		for (int d = 0; d < 16; d++) {
			values[v++] = values[d] * values[d];
		}
		int k = 1;
		for (int i = 0; i < 16; i++) {
			for (int j = k; j < 16; j++) {
				values[v++] = values[i] * values[j];
			}
			k += 1;	
		}
