...
import pacman.entries.pacman.PolynomialFeatures;
 ...
 */	
    ...
    public static ArrayList<Double> SXs =  new ArrayList<Double>();
   public static ArrayList<Double> QXs =  new ArrayList<Double>();
	...
	public static void Experiment1(){
		...
		//train params
		LENGTH = 1000; //how many train episodes
		//TEST = 10;
		...
		for (int i = 0; i < numcurves; i++) {
			train("independent_temp",0,false,false);
		}
		STUDENT = "depthQ";
		for (int i = 0; i < numcurves; i++) {
			train("independent_temp",0,false,false);
		...
		GraphExp1.main(SXs,QXs, numcurves);
		System.out.println("are we done");
	...
	public static RLPacMan create(String learner) {
		...
		boolean polyfeats = true; 
		FeatureSet studentProto = new PolynomialFeatures();

/*		if (STUDENT.startsWith("poly")) {
			FeatureSet studentProto = new PolynomialFeatures();
		}
		else {
	...
	public static void train(String learner, int start, boolean watchAtEnd, boolean watchDuring) {
		...
		for (int x=1; x<=LENGTH; x++) {
			...
			if (STUDENT == "depthS" || STUDENT == "polyS") {
				SXs.add(score);
			}
			if (STUDENT == "depthQ" || STUDENT == "polyQ") {
				QXs.add(score);
			}
